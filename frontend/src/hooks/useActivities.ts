...
export function useActivities() {
  ...
  const getActivitiesForChild = useCallback(
    async (
      childId: string,
      category?: string,
      tags?: string[]
  ...
  const scheduleActivity = useCallback(
    async (data: ScheduleActivityData): Promise<ActivityLog> => {
      try {
        setError(null);
        setLoading(true);
  ...
  const getUpcomingActivities = useCallback(
    async (childId: string): Promise<ActivityLog[]> => {
      try {
        setError(null);
        setLoading(true);
        ...
        const response = await fetch(
          `/api/activities/child/${childId}/upcoming`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const getCompletedActivities = useCallback(
    async (childId: string): Promise<ActivityLog[]> => {
      try {
        ...
        const response = await fetch(
          `/api/activities/child/${childId}/completed`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const generateSuggestions = useCallback(
    async (childId: string): Promise<Activity[]> => {
      try {
        setError(null);
        setLoading(true);
