...
export function useDevelopment() {
  ...
  const getMilestonesForChild = useCallback(
    async (childId: string, category?: string): Promise<Milestone[]> => {
      try {
        setError(null);
        setLoading(true);
  ...
  const getAchievedMilestones = useCallback(
    async (childId: string): Promise<MilestoneTracking[]> => {
      try {
        setError(null);
        setLoading(true);
        ...
        const response = await fetch(
          `/api/development/child/${childId}/milestones/achieved`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const getUpcomingMilestones = useCallback(
    async (childId: string): Promise<Milestone[]> => {
      try {
        setError(null);
        setLoading(true);
        ...
        const response = await fetch(
          `/api/development/child/${childId}/milestones/upcoming`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const generateReport = useCallback(
    async (childId: string): Promise<string> => {
      try {
        ...
        const response = await fetch(
          `/api/development/child/${childId}/report`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const getDevelopmentStats = useCallback(
    async (childId: string): Promise<DevelopmentStats> => {
      try {
        setError(null);
        setLoading(true);
        ...
        const response = await fetch(
          `/api/development/child/${childId}/stats`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
