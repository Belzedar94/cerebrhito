      ...
      res.send = function (body: any): Response {
        try {
          ...
          if (res.statusCode >= 200 && res.statusCode < 300) {
            cacheManager.set(cacheKey, data, options.type).catch(error => {
...
export function clearCacheMiddleware(cacheManager: CacheManager, type: string) {
  return async (req: Request, res: Response, next: NextFunction) => {
    // Store original send function
    const originalSend = res.send;
    ...
    res.send = function (body: any): Response {
      try {
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          if (operation) {
            cacheManager.invalidate(type, operation).catch(error => {
