...
export class LogAggregator {
        ...
        for (const entry of entries) {
          if (entry.metadata.statusCode) {
            ...
            if (entry.metadata.path) {
              endpointCounts[entry.metadata.path] = (endpointCounts[entry.metadata.path] || 0) + 1;
            }
  ...
  async exportLogs(query: LogQuery, format: 'json' | 'csv'): Promise<string> {
    try {
      ...
      const outputPath = path.join(this.logDir, `export_${Date.now()}.${format}`);
  ...
  private async processLogFile(filePath: string, query: LogQuery): Promise<LogEntry[]> {
    const entries: LogEntry[] = [];
    const fileStream = createReadStream(filePath);
    const rl = createInterface({
      input: filePath.endsWith('.gz') ? fileStream.pipe(createGunzip()) : fileStream,
      crlfDelay: Infinity,
    });
  ...
  private matchesQuery(entry: LogEntry, query: LogQuery): boolean {
    ...
    if (
      query.minDuration &&
      (!entry.metadata.duration || parseFloat(entry.metadata.duration) < query.minDuration)
    ) {
      return false;
    }
