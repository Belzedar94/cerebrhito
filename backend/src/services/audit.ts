...
export class AuditService implements IService {
  ...
  async getStatistics(
    ...
    try {
      ...
      const { count: successfulEvents } = await baseQuery
        .eq('status', 'success')
        .count();

      // Get failed events
      const { count: failedEvents } = await baseQuery
        .eq('status', 'failure')
        .count();
      ...
      const actionCounts = topActions?.reduce((acc, { action }) => {
      ...
      const resourceCounts = topResources?.reduce((acc, { resource_type }) => {
      ...
      const errorCounts = topErrors?.reduce((acc, { error }) => {
