...
const specializationSchema = z.string()
  ...
  .regex(/^[a-zA-Z\s\-']+$/, 'Specialization can only contain letters, spaces, hyphens, and apostrophes')
  .transform(val => val.trim());
...
export const childSchema = z.object({
  ...
  additional_languages: z.array(z.string()
    ...
    .optional()
    .default([]),
  development_concerns: z.array(z.string()
...
export const updateChildSchema = childSchema.partial().refine(
  data => Object.keys(data).length > 0,
  {
    message: 'At least one field must be provided for update'
  }
...
const activityDescriptionSchema = z.string()
...
export const activitySchema = z.object({
  ...
  materials_needed: z.array(materialSchema)
    .max(20, 'Cannot have more than 20 materials')
    .optional()
    .default([]),
  skills_developed: z.array(skillSchema)
  ...
  safety_notes: z.string()
    .max(1000, 'Safety notes must not exceed 1000 characters')
    .optional(),
...
export const activityLogSchema = z.object({
  ...
  media: z.array(z.object({
    ...
    .optional()
    .default([]),
  milestones_observed: z.array(z.object({
  ...
  skills_practiced: z.array(skillSchema)
    ...
    .optional()
    .default([]),
  challenges_faced: z.array(z.string()
...
// Milestone schemas
const milestoneNameSchema = z.string()
  .min(3, 'Milestone name must be at least 3 characters')
  .max(200, 'Milestone name must not exceed 200 characters')
  .transform(val => val.trim());

const milestoneDescriptionSchema = z.string()
  .min(10, 'Description must be at least 10 characters')
  .max(2000, 'Description must not exceed 2000 characters')
  .transform(val => val.trim());

const indicatorSchema = z.string()
...
export const milestoneSchema = z.object({
  ...
  supporting_activities: z.array(z.object({
    activity_id: idSchema,
    notes: z.string()
  ...
  prerequisites: z.array(idSchema)
    ...
    .optional()
    .default([]),
  next_milestones: z.array(idSchema)
  ...
  typical_age_range: z.object({
    ...
    data => data.min_months <= data.max_months,
    {
      message: 'Minimum age must be less than or equal to maximum age',
      path: ['min_months']
    }
...
export const updateMilestoneSchema = z.preprocess(
  ...
  z.object({
    ...
    typical_age_range: z.object({
      ...
      .optional()
      .refine(
        data => !data || data.min_months <= data.max_months,
        {
          message: 'Minimum age must be less than or equal to maximum age',
          path: ['min_months']
        }
...
export const aiMessageSchema = z.object({
  ...
  context: z.object({
    ...
    language: z.string()
      ...
      .optional(),
    voice_id: z.string()
      .uuid('Invalid voice ID format')
      .optional(),
    response_format: z.enum(['text', 'voice', 'both'], {
...
export const aiResponseSchema = z.object({
  ...
  suggested_activities: z.array(z.object({
    title: z.string()
    ...
    type: z.enum(['milestone', 'interaction', 'assessment'], {
      errorMap: () => ({ message: 'Activity type must be one of: milestone, interaction, assessment' })
    }),
    age_range: z.object({
      min_months: z.number()
        .int('Age must be a whole number')
    ...
    .optional(),
  development_insights: z.array(z.object({
    area: z.string()
