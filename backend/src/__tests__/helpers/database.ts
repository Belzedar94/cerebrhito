...
export const seedDatabase = async (client: SupabaseClient) => {
  try {
    ...
    const { error: activityLogError } = await client.from('activity_logs').insert({
      child_id: child.id,
    ...
    const { error: milestoneTrackingError } = await client.from('milestone_tracking').insert({
      child_id: child.id,
      milestone_id: milestone.id,
    });

    if (milestoneTrackingError) {
      throw milestoneTrackingError;
    }
    ...
    const { error: aiChatError } = await client.from('ai_chat_history').insert({
      user_id: user.id,
      child_id: child.id,
...
export const getTestData = async (client: SupabaseClient) => {
  try {
    ...
    const { data: child } = await client.from('children').select().eq('user_id', user.id).single();
    ...
    const { data: media } = await client.from('media').select().eq('child_id', child.id).single();
