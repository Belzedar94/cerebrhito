...
import { aiMessageSchema, aiResponseSchema, idSchema, dateSchema } from '../validation/schemas';
  ...
  validateRequest({
    body: z.object({
      ...
      preferredLanguage: z
        ...
        .optional(),
      responseFormat: z.enum(['text', 'voice', 'both']).default('text'),
      maxTokens: z
        .number()
...
router.post(
  ...
  validateRequest({
    body: z.object({
      ...
      category: z
        .enum(['accuracy', 'helpfulness', 'appropriateness', 'clarity', 'other'])
        .optional(),
      tags: z
        .array(
          z
